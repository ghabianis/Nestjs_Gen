  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="contractsoncandidate">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(contractsoncandidate)"
            :key="index"
          >
...
export default defineComponent({
  name: "contractsoncandidate-detail",
  components: {},
  setup() {
    const route = useRoute();
    const { contractsoncandidate } = storeToRefs(
      useContractsOnCandidateStore()
    );
    const { getContractsOnCandidateById } = useContractsOnCandidateStore();
    onMounted(async () => {
      const id = route?.params?.id as string;
      await getContractsOnCandidateById(id);
      setCurrentPageBreadcrumbs("contractsoncandidates", ["detail"]);
    });
