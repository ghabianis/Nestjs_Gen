      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${
                contractsoncandidateList[scope.$index].id
              }`"
              >{{ contractsoncandidateList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal &&
                modalId === contractsoncandidateList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () =>
                  handleRemoveEntity(contractsoncandidateList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  name: "contractsoncandidateList",
  components: { Pagination, ConfirmModal },
  setup() {
    const entityPluralName = "contractsoncandidates";
    const fieldsName = ref(["candidate", "contract"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { contractsoncandidateList, pagination, isLoading } = storeToRefs(
      useContractsOnCandidateStore()
    );
    const {
      fetchContractsOnCandidates,
      deleteContractsOnCandidate,
    } = useContractsOnCandidateStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteContractsOnCandidate(id);
    };
    ...
    const setItemsPerPage = async (event: any) => {
      await fetchContractsOnCandidates({
        skip: 0,
