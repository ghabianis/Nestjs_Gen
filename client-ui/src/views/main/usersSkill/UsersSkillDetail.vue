  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="usersskill">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(usersskill)"
            :key="index"
          >
...
export default defineComponent({
  name: "usersskill-detail",
  components: {},
  setup() {
    const route = useRoute();
    const { usersskill } = storeToRefs(useUsersSkillStore());
    const { getUsersSkillById } = useUsersSkillStore();
    onMounted(async () => {
      const id = route?.params?.id as string;
      await getUsersSkillById(id);
      setCurrentPageBreadcrumbs("usersskills", ["detail"]);
    });
