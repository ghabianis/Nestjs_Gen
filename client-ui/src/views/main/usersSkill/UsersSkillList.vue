      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${usersskillList[scope.$index].id}`"
              >{{ usersskillList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <el-button
              @click="
                () => handleOpenConfirmModal(usersskillList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === usersskillList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(usersskillList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "usersskills";
    const fieldsName = ref(["skill", "user"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { usersskillList, pagination, isLoading } = storeToRefs(
      useUsersSkillStore()
    );
    const { fetchUsersSkills, deleteUsersSkill } = useUsersSkillStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteUsersSkill(id);
    };
