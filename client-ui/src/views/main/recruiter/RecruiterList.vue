      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${recruiterList[scope.$index].id}`"
              >{{ recruiterList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <el-button
              @click="
                () => handleOpenConfirmModal(recruiterList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === recruiterList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(recruiterList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "recruiters";
    const fieldsName = ref(["entreprise", "user"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { recruiterList, pagination, isLoading } = storeToRefs(
      useRecruiterStore()
    );
    const { fetchRecruiters, deleteRecruiter } = useRecruiterStore();
