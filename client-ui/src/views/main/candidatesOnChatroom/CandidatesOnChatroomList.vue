      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${
                candidatesonchatroomList[scope.$index].id
              }`"
              >{{ candidatesonchatroomList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal &&
                modalId === candidatesonchatroomList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () =>
                  handleRemoveEntity(candidatesonchatroomList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  name: "candidatesonchatroomList",
  components: { Pagination, ConfirmModal },
  setup() {
    const entityPluralName = "candidatesonchatrooms";
    const fieldsName = ref(["chatroom", "candidate"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { candidatesonchatroomList, pagination, isLoading } = storeToRefs(
      useCandidatesOnChatroomStore()
    );
    const {
      fetchCandidatesOnChatrooms,
      deleteCandidatesOnChatroom,
    } = useCandidatesOnChatroomStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteCandidatesOnChatroom(id);
    };
    ...
    const setItemsPerPage = async (event: any) => {
      await fetchCandidatesOnChatrooms({
        skip: 0,
