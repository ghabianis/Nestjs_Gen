      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${feedbackList[scope.$index].id}`"
              >{{ feedbackList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/edit/${feedbackList[scope.$index].id}`"
              :underline="false"
              type="primary"
              class="me-3"
            >
            ...
            <el-button
              @click="
                () => handleOpenConfirmModal(feedbackList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === feedbackList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(feedbackList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "feedbacks";
    const fieldsName = ref(["contenu", "statue", "type", "offer", "user"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { feedbackList, pagination, isLoading } = storeToRefs(
      useFeedbackStore()
    );
    const { fetchFeedbacks, deleteFeedback } = useFeedbackStore();
