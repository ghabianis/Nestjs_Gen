<template>
  <div class="mx-auto mt-5 ms-4 me-4">
    <el-card shadow="never" class="card">
      ...
      <el-table
        :data="publicationsoncommunityList"
      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${
                publicationsoncommunityList[scope.$index].id
              }`"
              >{{ publicationsoncommunityList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal &&
                modalId === publicationsoncommunityList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
...
export default defineComponent({
  name: "publicationsoncommunityList",
  components: { Pagination, ConfirmModal },
  setup() {
    const entityPluralName = "publicationsoncommunities";
    const fieldsName = ref(["publication", "community"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { publicationsoncommunityList, pagination, isLoading } = storeToRefs(
      usePublicationsOnCommunityStore()
    );
    const {
      fetchPublicationsOnCommunities,
      deletePublicationsOnCommunity,
    } = usePublicationsOnCommunityStore();

    const handleRemoveEntity = async (id: string) => {
      await deletePublicationsOnCommunity(id);
    };
    ...
    const setItemsPerPage = async (event: any) => {
      await fetchPublicationsOnCommunities({
        skip: 0,
