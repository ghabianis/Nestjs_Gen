  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="comment">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(comment)"
            :key="index"
          >
...
export default defineComponent({
  ...
  setup() {
    ...
    const { comment } = storeToRefs(useCommentStore());
    const { getCommentById } = useCommentStore();
    onMounted(async () => {
      const id = route?.params?.id as string;
      await getCommentById(id);
      setCurrentPageBreadcrumbs("comments", ["detail"]);
    });
