      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${commentList[scope.$index].id}`"
              >{{ commentList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/edit/${commentList[scope.$index].id}`"
              :underline="false"
              type="primary"
              class="me-3"
            >
            ...
            <el-button
              @click="
                () => handleOpenConfirmModal(commentList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === commentList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(commentList[scope.$index]?.id)
              "
            >
...
import { useCommentStore } from "@/store/useCommentModule";
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "comments";
    const fieldsName = ref(["content", "user"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { commentList, pagination, isLoading } = storeToRefs(
      useCommentStore()
    );
    const { fetchComments, deleteComment } = useCommentStore();
