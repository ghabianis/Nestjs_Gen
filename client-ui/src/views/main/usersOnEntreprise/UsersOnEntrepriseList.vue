      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${
                usersonentrepriseList[scope.$index].id
              }`"
              >{{ usersonentrepriseList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal &&
                modalId === usersonentrepriseList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () =>
                  handleRemoveEntity(usersonentrepriseList[scope.$index]?.id)
              "
            >
...
import { useUsersOnEntrepriseStore } from "@/store/useUsersOnEntrepriseModule";
import Pagination from "@/components/shared/pagination/Pagination.vue";
const ConfirmModal = defineAsyncComponent(
  () => import("@/components/modals/ConfirmModal.vue")
);
export default defineComponent({
  name: "usersonentrepriseList",
  components: { Pagination, ConfirmModal },
  setup() {
    const entityPluralName = "usersonentreprises";
    const fieldsName = ref(["entreprise", "user"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { usersonentrepriseList, pagination, isLoading } = storeToRefs(
      useUsersOnEntrepriseStore()
    );
    const {
      fetchUsersOnEntreprises,
      deleteUsersOnEntreprise,
    } = useUsersOnEntrepriseStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteUsersOnEntreprise(id);
    };
