  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="followentreprise">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(followentreprise)"
            :key="index"
          >
...
export default defineComponent({
  name: "followentreprise-detail",
  components: {},
  setup() {
    const route = useRoute();
    const { followentreprise } = storeToRefs(useFollowEntrepriseStore());
    const { getFollowEntrepriseById } = useFollowEntrepriseStore();
    onMounted(async () => {
      const id = route?.params?.id as string;
      await getFollowEntrepriseById(id);
      setCurrentPageBreadcrumbs("followentreprises", ["detail"]);
    });
