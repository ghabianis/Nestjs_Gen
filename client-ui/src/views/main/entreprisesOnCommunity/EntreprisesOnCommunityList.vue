<template>
  <div class="mx-auto mt-5 ms-4 me-4">
    <el-card shadow="never" class="card">
      ...
      <el-table
        :data="entreprisesoncommunityList"
      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${
                entreprisesoncommunityList[scope.$index].id
              }`"
              >{{ entreprisesoncommunityList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal &&
                modalId === entreprisesoncommunityList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
...
export default defineComponent({
  name: "entreprisesoncommunityList",
  components: { Pagination, ConfirmModal },
  setup() {
    const entityPluralName = "entreprisesoncommunities";
    const fieldsName = ref(["entreprise", "community"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { entreprisesoncommunityList, pagination, isLoading } = storeToRefs(
      useEntreprisesOnCommunityStore()
    );
    const {
      fetchEntreprisesOnCommunities,
      deleteEntreprisesOnCommunity,
    } = useEntreprisesOnCommunityStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteEntreprisesOnCommunity(id);
    };
    ...
    const setItemsPerPage = async (event: any) => {
      await fetchEntreprisesOnCommunities({
        skip: 0,
