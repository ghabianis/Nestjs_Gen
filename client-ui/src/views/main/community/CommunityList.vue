      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${communityList[scope.$index].id}`"
              >{{ communityList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <el-button
              @click="
                () => handleOpenConfirmModal(communityList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === communityList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(communityList[scope.$index]?.id)
              "
            >
...
import { useCommunityStore } from "@/store/useCommunityModule";
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "communities";
    ...
    const { communityList, pagination, isLoading } = storeToRefs(
      useCommunityStore()
    );
    const { fetchCommunities, deleteCommunity } = useCommunityStore();
