<template>
  <div class="d-flex gap-5">
      ...
      <div class="community-posts-body__container d-flex gap-4 ">
        ...
        <div class="community-posts-card col-8">
          <InfinityScroll @bottom="handleBottom">
                <CommunityPostCard v-for="post in postList" :key="post.publicationId" :post="post"/>
            </InfinityScroll>
    ...
    <div >
      <AsideCommunityCard :isSolo="false" />
    </div>
  </div>
</template>
...
import AsideCommunityCard from "@/components/cart/recruiterCard/AsideCommunityCard.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const { postList,isLoading ,pagination,paginationEnd,searchInput} = storeToRefs(useCommunityPosts());
   ...
   const handleBottom = async () => {
    ...
    console.log(!paginationEnd.value,"!isLoading.value && !paginationEnd")
          if(!isLoading.value && !paginationEnd.value ){
            ...
            await getPost({communityId:id,pagination: {
    ...
    return {
      postList,handleBottom,searchInput,handelInput
    };
  },
});
</script>
