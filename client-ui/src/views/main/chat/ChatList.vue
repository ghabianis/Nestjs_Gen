      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${chatList[scope.$index].id}`"
              >{{ chatList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/edit/${chatList[scope.$index].id}`"
              :underline="false"
              type="primary"
              class="me-3"
            >
            ...
            <el-button
              @click="() => handleOpenConfirmModal(chatList[scope.$index]?.id)"
              icon="Delete"
              circle
              plain
              type="danger"
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === chatList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(chatList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "chats";
    const fieldsName = ref(["candidate", "entreprise", "messages"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { chatList, pagination, isLoading } = storeToRefs(useChatStore());
    const { fetchChats, deleteChat } = useChatStore();
