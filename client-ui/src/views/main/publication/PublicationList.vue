      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${publicationList[scope.$index].id}`"
              >{{ publicationList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/edit/${
                publicationList[scope.$index].id
              }`"
              :underline="false"
              type="primary"
              class="me-3"
            ...
            <el-button
              @click="
                () => handleOpenConfirmModal(publicationList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === publicationList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(publicationList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "publications";
    ...
    const { publicationList, pagination, isLoading } = storeToRefs(
      usePublicationStore()
    );
    const { fetchPublications, deletePublication } = usePublicationStore();
