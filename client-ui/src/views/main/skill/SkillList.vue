      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${skillList[scope.$index].id}`"
              >{{ skillList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/edit/${skillList[scope.$index].id}`"
              :underline="false"
              type="primary"
              class="me-3"
            >
            ...
            <el-button
              @click="() => handleOpenConfirmModal(skillList[scope.$index]?.id)"
              icon="Delete"
              circle
              plain
              type="danger"
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === skillList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(skillList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "skills";
    const fieldsName = ref(["name", "usersSkills"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { skillList, pagination, isLoading } = storeToRefs(useSkillStore());
    const { fetchSkills, deleteSkill } = useSkillStore();
