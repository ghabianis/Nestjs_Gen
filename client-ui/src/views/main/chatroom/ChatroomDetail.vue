  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="chatroom">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(chatroom)"
            :key="index"
          >
...
export default defineComponent({
  ...
  setup() {
    ...
    const { chatroom } = storeToRefs(useChatroomStore());
    const { getChatroomById } = useChatroomStore();
    onMounted(async () => {
      const id = route?.params?.id as string;
      await getChatroomById(id);
      setCurrentPageBreadcrumbs("chatrooms", ["detail"]);
    });
