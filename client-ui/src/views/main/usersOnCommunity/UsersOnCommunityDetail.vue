  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="usersoncommunity">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(usersoncommunity)"
            :key="index"
          >
...
export default defineComponent({
  name: "usersoncommunity-detail",
  components: {},
  setup() {
    const route = useRoute();
    const { usersoncommunity } = storeToRefs(useUsersOnCommunityStore());
    const { getUsersOnCommunityById } = useUsersOnCommunityStore();
    onMounted(async () => {
      const id = route?.params?.id as string;
      await getUsersOnCommunityById(id);
      setCurrentPageBreadcrumbs("usersoncommunities", ["detail"]);
    });
