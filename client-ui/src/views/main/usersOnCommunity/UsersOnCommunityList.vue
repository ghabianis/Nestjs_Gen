      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${
                usersoncommunityList[scope.$index].id
              }`"
              >{{ usersoncommunityList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <el-button
              @click="
                () =>
                  handleOpenConfirmModal(usersoncommunityList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === usersoncommunityList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(usersoncommunityList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  name: "usersoncommunityList",
  components: { Pagination, ConfirmModal },
  setup() {
    const entityPluralName = "usersoncommunities";
    const fieldsName = ref(["community", "user"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { usersoncommunityList, pagination, isLoading } = storeToRefs(
      useUsersOnCommunityStore()
    );
    const {
      fetchUsersOnCommunities,
      deleteUsersOnCommunity,
    } = useUsersOnCommunityStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteUsersOnCommunity(id);
    };
