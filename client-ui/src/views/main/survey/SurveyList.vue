      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${surveyList[scope.$index].id}`"
              >{{ surveyList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/edit/${surveyList[scope.$index].id}`"
              :underline="false"
              type="primary"
              class="me-3"
            >
            ...
            <el-button
              @click="
                () => handleOpenConfirmModal(surveyList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === surveyList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(surveyList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "surveys";
    const fieldsName = ref(["title", "content", "entreprise"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { surveyList, pagination, isLoading } = storeToRefs(useSurveyStore());
    const { fetchSurveys, deleteSurvey } = useSurveyStore();
