  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="socialinterraction">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(socialinterraction)"
            :key="index"
          >
...
export default defineComponent({
  name: "socialinterraction-detail",
  components: {},
  setup() {
    const route = useRoute();
    const { socialinterraction } = storeToRefs(useSocialinterractionStore());
    const { getSocialinterractionById } = useSocialinterractionStore();
    onMounted(async () => {
      const id = route?.params?.id as string;
      await getSocialinterractionById(id);
      setCurrentPageBreadcrumbs("socialinterractions", ["detail"]);
    });
