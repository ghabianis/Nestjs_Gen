      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${
                socialinterractionList[scope.$index].id
              }`"
              >{{ socialinterractionList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal &&
                modalId === socialinterractionList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () =>
                  handleRemoveEntity(socialinterractionList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  name: "socialinterractionList",
  components: { Pagination, ConfirmModal },
  setup() {
    const entityPluralName = "socialinterractions";
    const fieldsName = ref(["type", "publication", "user"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { socialinterractionList, pagination, isLoading } = storeToRefs(
      useSocialinterractionStore()
    );
    const {
      fetchSocialinterractions,
      deleteSocialinterraction,
    } = useSocialinterractionStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteSocialinterraction(id);
    };
    ...
    const setItemsPerPage = async (event: any) => {
      await fetchSocialinterractions({
        skip: 0,
