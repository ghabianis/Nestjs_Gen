  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="offersoncommunity">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(offersoncommunity)"
            :key="index"
          >
...
export default defineComponent({
  name: "offersoncommunity-detail",
  components: {},
  setup() {
    const route = useRoute();
    const { offersoncommunity } = storeToRefs(useOffersOnCommunityStore());
    const { getOffersOnCommunityById } = useOffersOnCommunityStore();
    onMounted(async () => {
      const id = route?.params?.id as string;
      await getOffersOnCommunityById(id);
      setCurrentPageBreadcrumbs("offersoncommunities", ["detail"]);
    });
