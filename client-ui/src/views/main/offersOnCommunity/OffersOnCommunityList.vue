      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${
                offersoncommunityList[scope.$index].id
              }`"
              >{{ offersoncommunityList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal &&
                modalId === offersoncommunityList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () =>
                  handleRemoveEntity(offersoncommunityList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  name: "offersoncommunityList",
  components: { Pagination, ConfirmModal },
  setup() {
    const entityPluralName = "offersoncommunities";
    const fieldsName = ref(["offer", "community"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { offersoncommunityList, pagination, isLoading } = storeToRefs(
      useOffersOnCommunityStore()
    );
    const {
      fetchOffersOnCommunities,
      deleteOffersOnCommunity,
    } = useOffersOnCommunityStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteOffersOnCommunity(id);
    };
    ...
    const setItemsPerPage = async (event: any) => {
      await fetchOffersOnCommunities({
        skip: 0,
